...
const QuickActionTile: React.FC<QuickActionTileProps> = ({ 
  ...
  const content = (
    <div className="quick-action-card">
      <div className="flex items-center space-x-3">
        <div className="p-3 bg-moses-light rounded-lg">
          <span className="text-2xl">{icon}</span>
        </div>
        <div className="flex-1 text-left">
          <h3 className="font-bold text-sm uppercase tracking-wide" style={{ color: '#ffffff' }}>{title}</h3>
          <p className="text-xs mt-1" style={{ color: 'rgba(255, 255, 255, 0.8)' }}>{description}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <a href={href} className="block" style={{ color: '#ffffff', textDecoration: 'none' }}>
      {content}
    </a>
  );
};
